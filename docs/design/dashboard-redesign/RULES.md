# 화면이 따르는 계산 규칙 (9/19 디자인 세션 시점 — 역사 문서)

> **저장소 주(2026-09-20)**: 이 문서는 디자인 세션이 사용자와 하나씩 확정한 **그때의 규칙**이다. 제품으로 이식하면서 여러 곳이
> 바뀌었다(50% 추정 → 행 단위 배치 창 배분 D56·D58, 대기 상한 240분 없음 D44, 하루 분모 D57, Test Lot D64, 중단 Report D52 …).
> **제품의 현재 규칙은 `CLAUDE.md` 의 '레이아웃' 절이고, 이 문서와 다른 곳의 목록은 [`STATUS.md`](STATUS.md) 에 있다.**
> 아래 본문은 원문 그대로 둔다(`scripts/make_aoi_data.js` 와 함께 legacy 프로필 동일성의 맥락이므로).
