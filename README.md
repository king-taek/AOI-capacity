...
가동률 = (Scan + Rescan) ÷ 24시간. 수집한 날(오늘)만 00:00 부터 그날 마지막 기록까지로 나눕니다(아직 오지 않은 시간을 대기로 세지 않기 위해). '오늘' 은 브라우저의 오늘이 아니라 **수집 시각의 날짜**입니다 — 같은 파일을 며칠 뒤 열어도 숫자가 같습니다(화면 위에 '2026-09-18 12:59 수집 기준').
Lot 이름에 `TEST` 가 있는 Test 가동은 분모에는 들어가고 분자에서만 뺍니다. 같은 자재(Job 병합 키 · Lot · Wafer ID)를 앞선 시도가 Pass 였는데 다시 스캔한 것은 장비와 무관하게 **Rescan** 이고, 앞선 시도가 Error 였으면 정상 Scan 입니다. Rescan 도 가동시간에 들어갑니다.
Error 는 **Lot 단위**로 셉니다(같은 Lot 의 같은 유형은 1건). Error 뒤 다음 기록까지의 공백은 '에러 후 대기' 로 보고 그날 마지막 Error 는 관측 종료(로드한 데이터의 마지막 시각)까지만 셉니다.
시각이 확인된 기록이 50% 미만인 날은 Report 의 배치 시작·종료 시각으로 가동률을 **추정**합니다. 목록에서는 같은 모양이고, 장비 팝업에서 '(추정)' 과 시각 확인 비율을 보여 줍니다.
