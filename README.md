...
| Error 분석 | 기간(파일 전체/최근 7·30개 날짜/직접) · 장비 · Job(원문 검색) 을 좁혀 **기간별 · 장비별 · Job별** Error 사건을 보고, 행을 누르면 원문 근거(Job·Lot·Wafer·원인·결과·시간 근거·Report 열기)로 내려간다. 세부 설정에서 포함 기록·시간 근거·저장된 보기·설정 JSON |
| 데이터 품질 | 시간으로 바꾸지 않은 기록(INI 없음·덮어써짐·소유권 보류·Slot 사건·실패 배치·Job 미확인)을 사유별로 세고 원문 근거 행으로 내려간다 |
| 추이 | 일 · 주 · 월 막대. 선택한 기간의 값만 보여 주며 전 기간 대비는 두지 않습니다 |
| 장비 비교 | 선택한 날 · 최근 7일 · 최근 30일 기준 장비 순위 |
| 설정 | 주의 장비 기준, HTML 로 저장. 바꾼 값은 **그 브라우저에만** 저장되며 원본 파일이나 수집 설정에는 반영되지 않습니다 |

가동률 = 가동시간(WaferStartTime~WaferEndTime 합) ÷ 24시간. 오늘은 00:00 부터 그날 마지막 스캔까지로 나눕니다(아직 오지 않은 시간을 미가동으로 세지 않기 위해). '오늘' 은 브라우저의 오늘이 아니라 **수집 시각의 날짜**입니다 — 같은 파일을 며칠 뒤 열어도 숫자가 같습니다(화면 위에 '9/18 12:59 수집 기준' 으로 표시).
...
Error Wafer 종료부터 다음 Wafer 시작까지의 공백은 '정지(추정)' 으로 봅니다(장비 이벤트 로그가 아닙니다).
`Aborted.`·`Wafer aborted by user.` 로 끝난 중단은 사람이 세운 것이라 Error 로 세지 않고 그 뒤 공백도 정지로 돌리지 않습니다. 다만 스캔이 끝나지 않았으므로 실가동에서는 빠지고 '중단' 으로 따로 보여 줍니다.
