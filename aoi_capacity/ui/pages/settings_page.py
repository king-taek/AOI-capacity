...
class SettingsPage(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        # 표시
        card, cl = _card(self, i18n.KO.SET_DISPLAY_TITLE)
        ...
        # 수집 기간 — 처음 보는 장비를 얼마나 거슬러 읽을지(장비를 새로 넣으면 이 기간이 적용된다)
        card, cl = _card(self, i18n.KO.SET_COLLECT_RANGE)
        g = QGridLayout()
        g.setHorizontalSpacing(8)
        self._backfill = QSpinBox(card)
        self._backfill.setRange(1, 3650)
        self._backfill.setSuffix(i18n.KO.SET_BACKFILL_SUFFIX)
        ...
        # 9/23: 결과 HTML 이 이 크기를 넘으면 기간별로 나눈다(0 = 안 나눔) — prefs.split_mb → cfg split_mb
        self._split = QSpinBox(card)
        self._split.setRange(0, 2000)
        self._split.setSuffix(i18n.KO.SET_SPLIT_MB_SUFFIX)
        self._split.setValue(int(p.split_mb))
        self._split.setToolTip(i18n.KO.SET_SPLIT_MB_HELP)
        g.addWidget(QLabel(i18n.KO.SET_SPLIT_MB, card), 1, 0)
        g.addWidget(self._split, 1, 1)
        g.setColumnStretch(6, 1)
        cl.addLayout(g)
        shl = QLabel(i18n.KO.SET_SPLIT_MB_HELP, card)
        shl.setProperty("role", "muted")
        shl.setWordWrap(True)
        cl.addWidget(shl)
        hlp = QLabel(i18n.KO.SET_COLLECT_RANGE_HELP, card)
        hlp.setProperty("role", "help")
        ...
        self._backfill.valueChanged.connect(lambda v: prefs.patch(backfill_days=int(v)))
        self._retention.valueChanged.connect(lambda v: prefs.patch(retention_days=int(v)))
        self._workers.valueChanged.connect(lambda v: prefs.patch(read_workers=int(v)))
        self._split.valueChanged.connect(lambda v: prefs.patch(split_mb=int(v)))
