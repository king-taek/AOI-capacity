...
DEV_PAGE_HELP = ("한 줄이 장비 하나입니다. NAS경로는 드라이브 문자(X:\\)나 주소(\\\\10.142.80.90\\공유) 어느 쪽이든 되고, "
                 ...
                 "* 이면 그 안의 장비 폴더를 자동 등록합니다(수집 범위 제한 중에는 공유를 나열하지 않고 허용 목록의 장비 이름만 확인합니다). "
...
COLLECT_PAGE_SUB = "지금 상황에 맞는 수집을 고르고 시작하세요. 대부분은 '평소 수집' 하나면 됩니다."
# 상황별 수집 카드(9/23 — "어떤 상황에 어떤 옵션을 눌러야 하는지 직관적으로"): 제목 · 이럴 때 · 무엇을 하나 · 걸리는 시간
COLLECT_PICK_TITLE = "무엇을 할까요?"
COLLECT_TROUBLE_EYEBROW = "결과가 이상하거나 비어 있을 때만"
MODE_NORMAL_TITLE = "평소 수집"
MODE_NORMAL_WHEN = "매일 · 교대 때마다 누르는 기본 수집"
MODE_NORMAL_WHAT = "지난 수집 뒤 새로 생긴 Report 만 읽습니다. 수집을 며칠 쉬었어도 이것으로 이어서 읽습니다."
MODE_FIRST_TITLE = "처음 수집"
MODE_FIRST_WHEN = "이 PC 에서 처음 수집할 때(자동으로 이것이 됩니다)"
MODE_FIRST_WHAT_FMT = "모든 장비의 최근 {days}일 Report 를 전부 읽습니다. 처음 한 번만이고, 장비 수에 따라 수십 분 걸릴 수 있습니다."
MODE_BACKFILL_TITLE = "빠진 날 채우기"
MODE_BACKFILL_WHEN = "결과에 비어 있는 날 · 장비가 있을 때(예: NAS 가 잠시 끊겼던 날)"
MODE_BACKFILL_WHAT_FMT = "최근 {days}일 안에서 아직 읽지 않은 Report 를 모두 찾아 읽습니다. 이미 읽은 것은 건너뜁니다."
MODE_REFRESH_TITLE = "최근 며칠 다시 읽기"
MODE_REFRESH_WHEN = "최근 결과가 이상하거나, 장비에서 Report 를 다시 저장했을 때"
MODE_REFRESH_WHAT = "이미 읽은 것도 이 기간은 다시 읽습니다. 그 이전 기록은 그대로 둡니다."
MODE_REFRESH_DAYS_PREFIX = "최근"
MODE_REFRESH_DAYS_SUFFIX = " 일"
MODE_RECOVER_TITLE = "시간 미확인 복구"
MODE_RECOVER_WHEN = "결과 화면에 'INI 없음 · 시간 미확인' 이 많을 때(Scanresult 백업 폴더가 새로 생겼거나 업데이트 뒤)"
MODE_RECOVER_WHAT = "INI 를 못 찾았던 Report 만 다시 읽어 시각을 되찾습니다. 새 Report 도 함께 읽습니다."
MODE_REBUILD_TITLE = "전체 다시 만들기"
MODE_REBUILD_WHEN = "업데이트 안내가 재수집을 권하거나, 결과 전체가 이상할 때"
MODE_REBUILD_WHAT_FMT = "보관 기간 {days}일을 전부 새로 읽어 검증한 뒤 바꿉니다. 실패하면 기존 결과를 그대로 둡니다."
MODE_BADGE_REC = "추천"
MODE_BADGE_FAST = "빠름"
MODE_BADGE_MEDIUM = "몇 분"
MODE_BADGE_SLOW = "오래 걸림 · 수십 분"
MODE_BADGE_TARGET_FMT = "대상 Report {n}개"
MODE_BADGE_NONE = "대상 없음"
MODE_DISABLED_FIRST = "처음 수집을 마친 뒤에 쓸 수 있습니다."
MODE_STALE_FMT = "마지막 수집이 {days}일 전입니다 — 이번엔 폴더 전체를 훑어 평소보다 조금 더 걸립니다."
MODE_RUN_FMT = "{title} 시작"
...
COLLECT_RESULT_PARTS_FMT = "결과가 커서 기간별 {n}개 파일로 나눴습니다(각각 따로 열립니다). 옛 기간 파일: {names}"
COLLECT_SAVE_TITLE = "저장 위치"
...
COLLECT_RESULT_NONE = "아직 수집한 적이 없습니다 — 위에서 수집을 먼저 시작하세요."
...
SET_READ_WORKERS = "NAS 마다 동시에 읽기"
SET_READ_WORKERS_SUFFIX = " 개씩"
SET_SPLIT_MB = "결과 파일 나누기 기준"
SET_SPLIT_MB_SUFFIX = " MB 넘으면"
SET_SPLIT_MB_HELP = "결과 HTML 이 이 크기를 넘으면 기간별 여러 파일로 나눕니다(메신저·메일 첨부 한도 대비). 0 이면 나누지 않습니다."
SET_COLLECT_RANGE_HELP = ("처음 보는 장비는 이 기간 안의 Report 를 전부 읽습니다. 그다음부터는 새 Report 만 읽습니다. "
                          "기간을 늘리면 첫 수집이 그만큼 오래 걸립니다.\n"
                          "NAS 읽기는 기다리는 시간이 대부분이라 NAS 마다 이 개수씩 동시에 읽습니다(NAS 여러 대는 함께). NAS 가 되레 느려지면 숫자를 낮추세요(1 = 한 줄로).")
SET_DATA_DIR = "데이터 폴더"
SET_VERSION = "버전"
...
SET_UTIL_DEFINITION = ("가동률 = (Scan + Rescan 시간) ÷ 24시간. 수집한 날(오늘)만 00:00 부터 모든 장비의 마지막 기록까지로 나눕니다 "
                       ...
                       "Rescan 은 같은 자재(Job · Lot · Wafer ID)의 앞선 시도가 Pass 였을 때만이고, Error·Skipped 뒤의 재시도는 Scan 입니다. "
