...
SET_UTIL_DEFINITION = ("가동률 = (Scan + Rescan 시간) ÷ 24시간. 수집한 날(오늘)만 00:00 부터 그날 마지막 기록까지로 나눕니다 "
                       "('오늘' 은 수집 시각의 날짜 — 며칠 뒤 열어도 같은 숫자). Test Lot 은 분모에만 들어갑니다. "
                       "Error 는 Lot 단위로 세고, Error 뒤 다음 기록까지의 공백을 '에러 후 대기' 로 봅니다. "
                       "시각이 확인된 기록이 50% 미만인 날은 Report 의 배치 시작·종료 시각으로 추정하고 상세 팝업에 '(추정)' 을 붙입니다.")
